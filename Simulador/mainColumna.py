...
caja = box(pos = vector(0.0,0.0,0.0), size = vector(4.0,3.0,3.0))
caja.opacity = 0.1
...
esfera = sphere(pos = vector(2.0,-1.5,0), radius = 0.4)
esfera.opacity = 1.0

cilindro = cylinder(pos = vector(1.0,-1.5,0.75), axis = vector(0.0,2.0,0.0), radius = 0.3)
cilindro.opacity = 1.0

cubo = box(pos = vector(1.0,-0.5,-0.75), size = vector(0.5,2.0,0.5))
cubo.opacitu = 1.0
...
sistemaGenerado = generaColumna(posicionInicial, 0.1, 10, 1, 10, separacion, velocidadInicial, radio)
...
while cont < 7500:
    ...
    if cont <= tiempoTotalIntervalo and cont%intervalo == 0 and cont>0:
        ...
        nuevaColumna = generaColumna(posicionInicial, 0.1, 10, 1, 10, separacion, velocidadInicial, radio)
