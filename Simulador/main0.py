...
caja = box(pos = vector(0.0,0.0,0.0), size = vector(3.0,3.0,3.0))
caja.opacity = 0.1
...
esfera = sphere(pos = vector(3.0,-1.5,0), radius = 0.4)
esfera.opacity = 1.0

cilindro = cylinder(pos = vector(3.0,-1.5,0.75), axis = vector(0.0,2.0,0.0), radius = 0.3)
cilindro.opacity = 1.0

cubo = box(pos = vector(3.0,-0.5,-0.75), size = vector(0.5,2.0,0.5))
cubo.opacitu = 1.0
...
posicionInicial = vector(0.75, -1.0, -1.0)
velocidadInicial = vector(2.5, -1.0, 0.75)
...
sistemaGenerado = generaColumna(posicionInicial, 0.1, 5, 1, 5, separacion, velocidadInicial, radio)
...
while cont < 7500:
    ...
    if cont <= tiempoTotalIntervalo and cont%intervalo == 0 and cont>0:
        nuevaColumna = SistemaParticulas()
        nuevaColumna = generaColumna(posicionInicial, 0.1, 5, 1, 5, separacion, velocidadInicial, radio)
        sistemaGenerado.agregaSistemaParticulas(nuevaColumna)
        cantidadParticulas = sistemaGenerado.getNumeroParticulas()
    ...
    if cont%15 == 0:
        exportador.exportar("simple", nFrame, sistema, radio)
        nFrame += 1
