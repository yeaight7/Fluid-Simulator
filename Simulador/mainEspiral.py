...
posicionInicial = vector(0.0, 0.0 ,0.0)
...
intervalo = int(radio * 1200)
vecesGenera = 100
tiempoTotalIntervalo = intervalo * vecesGenera
momento = pi / 10

sistemaGenerado = SistemaParticulas()
sistemaGenerado = generaColumnaEspiral(posicionInicial, 0.1, 8, 1, separacion, velocidadInicial, radio, momento)
...
while cont < 7500:
    ...
    if cont <= tiempoTotalIntervalo and cont%intervalo == 0 and cont>0:
        ...
        nuevaColumna = generaColumnaEspiral(posicionInicial, 0.1, 8, 1, separacion, velocidadInicial, radio, momento)
